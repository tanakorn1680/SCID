 ...
 * รองรับ qty > 1 โดยสร้าง N orders แยกกัน (1 order = 1 ไอดี)
 * แต่ละ order มี unique_amount เป็นของตัวเอง
 *
 * หมายเหตุ: ไม่รวมเป็น order เดียวเพราะ deliver_order() RPC ส่งไอดีทีละ 1
 * และ unique_amount ต้องผูกกับราคาต่อชิ้น เพื่อให้ Bank Amount Reader จับคู่ได้
 */
export async function createOrder(profile, { product_key, qty = 1 }) {
  ...
  if (!Number.isInteger(quantity) || quantity < 1 || quantity > 10) {
    return { httpStatus: 400, body: { success: false, error: 'จำนวนต้องอยู่ระหว่าง 1-10' } };
  }

  // ตรวจสต็อกว่าพอก่อนสร้าง order (non-blocking check — ไม่ lock)
  const { count: stockCount, error: stockErr } = await supabaseAdmin
    .from('inventory')
    .select('id', { count: 'exact', head: true })
  ...
  if ((stockCount ?? 0) < quantity) {
    return {
      ...
      body: {
        ...
        error: quantity === 1
          ? 'สินค้าหมดชั่วคราว'
          : `สินค้าในคลังไม่พอ (มี ${stockCount ?? 0} ชิ้น ต้องการ ${quantity} ชิ้น)`,
      },
    };
  }

  // สร้าง unique_amount สำหรับแต่ละ order แบบ parallel
  const uniqueAmounts = await Promise.all(
    Array.from({ length: quantity }, () => generateUniqueAmount(product.price))
  );

  // insert N rows
  const rows = uniqueAmounts.map((uniqueAmount) => ({
    user_id:       profile.id,
  ...
  const { data: createdOrders, error } = await supabaseAdmin
    .from('orders')
    .insert(rows)
  ...
  return { httpStatus: 200, body: { success: true, data: createdOrders } };
}
