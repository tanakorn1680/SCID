...
export async function adminDeleteMethod({ id }) {
  ...
  // ตรวจว่ามี orders อ้าง payment_method_id นี้อยู่ไหม
  ...
  if (count > 0) {
    // มี orders อ้างอยู่ → ปิดใช้งานแทนลบ เพื่อไม่ให้ FK constraint พัง
    ...
    return {
      ...
      body: {
        success: true,
        deactivated: true,
        message: 'ช่องทางนี้มีออเดอร์ผูกอยู่ ' + count + ' รายการ จึงปิดใช้งานแทนการลบ',
      },
    };
  }

  // ไม่มี orders อ้าง → ลบได้เลย
  const { error } = await supabaseAdmin.from('payment_methods').delete().eq('id', id);
  if (error) throw error;

  return { httpStatus: 200, body: { success: true, deactivated: false } };
}
