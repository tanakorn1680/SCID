...
export const orders = {
  // qty: จำนวนชิ้น (default 1) — backend จะสร้าง N orders แยกกัน
  // คืน { data: [order, ...] } เสมอ (array แม้ qty=1)
  async create(productKey, qty = 1) {
    return apiFetch('/api/orders?action=create', {
      method: 'POST',
      body: JSON.stringify({ product_key: productKey, qty }),
    });
  },
